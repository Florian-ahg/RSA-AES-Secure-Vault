...
void AES::keyExpansion(const unsigned char* key)
{
    ...
    // Copy the original key into the first 16 bytes
    for (int i = 0; i < 16; i++)
    ...
    for (int i = 16; i < 176; i += 4) {
        ...
        if (i % 16 == 0) {
            ...
            subWord(temp);
            for (int j = 0; j < 4; j++)
                temp[j] ^= Rcon[(i / 16) - 1][j]; 
        }

        for (int j = 0; j < 4; j++)
            expandedKeys[i + j] = expandedKeys[i - 16 + j] ^ temp[j];
    }
}
...
std::vector<unsigned char> AES::decrypt(const std::vector<unsigned char>& ciphertext)
{
    ...
    if (ciphertext.size() % 16 != 0)
        throw std::invalid_argument("Ciphertext size must be a multiple of 16 bytes");
...
void AES::addRoundKey(unsigned char* state, int round)
{
    for (int i = 0; i < 16; i++)
...
void AES::mixColumns(unsigned char* state)
{
    for (int col = 0; col < 4; col++) {
        int idx = col * 4;
        unsigned char s0 = state[idx];
        unsigned char s1 = state[idx + 1];
        unsigned char s2 = state[idx + 2];
        unsigned char s3 = state[idx + 3];

        state[idx] = gmul(0x02, s0) ^ gmul(0x03, s1) ^ s2 ^ s3;
